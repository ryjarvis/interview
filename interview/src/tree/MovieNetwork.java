...
/*	Difficulty:
	...
	Finally,Movies numbered [0,1,2,3,4,6,7] are associated with movie 5, and the order which according to their rating from hig

	*/
	
	public static int[] topKMovie(int[] rating, int[][] G, int S, int K){
		if(G[S].length==0){
			return new int[0];
		}
		PriorityQueue<Integer> pq=new PriorityQueue<>((a,b)->(rating[a]-rating[b]));
		dfs(G,K,S,new boolean[rating.length],rating,pq);
	...
	public static void dfs(int[][]g,int k,int s,boolean[]seen,int[]rating,PriorityQueue<Integer> pq){
		seen[s]=true;
		for(int i:g[s]){
			if(seen[i]){
				continue;
			...
			else if(rating[pq.peek()]<rating[i]){
				pq.poll();
				pq.add(i);
			}
			dfs(g,k,i,seen,rating,pq);
		}
	}
	...
	public static void main(String[] args) {
		...
		int[]res=topKMovie(rating, g,5, 3);
