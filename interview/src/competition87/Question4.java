...
public class Question4 {
	...
	public static int shortestPathLength5(int[][] graph) {
		int N = graph.length;
		...
		for (int x = 0; x < N; ++x) {
			queue.offer(new State2(1 << x, x));
			dist[1 << x][x] = 0;
		}

		while (!queue.isEmpty()) {
			State2 node = queue.poll();
			...
			for (int child : graph[node.head]) {
				...
				if (d + 1 < dist[cover2][child]) {
					...
					queue.offer(new State2(cover2, child));
	...
	public static void main(String[] args) {
		...
		int res = shortestPathLength5(graph);
	}
